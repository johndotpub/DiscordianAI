...
class TestSendFormattedMessage:
    ...
    async def test_send_formatted_message_embed_failure_fallback(self):
        ...
        channel = MagicMock(spec=discord.TextChannel)
        # HTTPException requires response parameter
        http_error = discord.HTTPException(response=MagicMock(), message="Embed failed")
        ...
        assert channel.send.call_args_list[1][1]["suppress_embeds"] == False
    ...
    async def test_no_duplicate_messages_sent(self):
        ...
        await send_formatted_message(channel, "Test", {"logger": MagicMock()}, embed_data=embed_data)
